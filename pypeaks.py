...
class FitPeaks():
    ...
    def background(self):
        ...
        # This function calculates the background spectrum in the input histogram, returned as a histogram.
        hb = self.s.Background(self.histogram, 20, "same")
    ...
    def n_peakstofit(self):  # Loop on all found peaks. Eliminate peaks at the background level
        ...
        for xpeak in (self.xpeaks):
            ...
            # compares if peak is over the background or not
            if (abs(ypeak-TMath.Sqrt(ypeak)) > self.fline.Eval(xpeak)):
                # height,mean,sigma;initial seeds for the fitting
                self.par = append(self.par, [ypeak, xpeak, 100])
                n_peakstofit += 1
