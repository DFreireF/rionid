...
class ImportData():
    ...
    def samples(self):        
        ...
        for k in range(0, len(self.m)):
            ...
            self.SRRF.append(1-1/self.GammaT/self.GammaT*(self.moq[k]-moq_Rel)/moq_Rel)
            # 2. simulated revolution frequency
            self.SRF.append(self.SRRF[k]*Frequence_Rel*self.Harmonic)

        print('Brho inicial',self.BRho)
        self.BRhoCorrection()
        print('Brho final',self.BRho)
    ...
    def tominimize(self,x):#function to minimize (x=Brho); yup, it's big
        tominimize=abs(self.ff[self.pp.argmax()]-((1-1/self.GammaT/self.GammaT*(self.moq[self.aux]-moq_Rel)/moq_Rel)*((AMEData.CC*(sqrt((sqrt(pow(x*self.RefQ*AMEData.CC/self.m[self.aux],2))*(sqrt(pow(x*self.RefQ*AMEData.CC/self.m[self.aux],2))-1)/(sqrt(pow(x*self.RefQ*AMEData.CC/self.m[self.aux],2))))/self.ring.circumference)*self.Harmonic))
        return tominimize
